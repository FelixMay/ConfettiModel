...
)                
{
   ...
   so.file <- "Confetti2_01_12_norm.dll"
...
#str(check)
# check$mBD_tot/21000
# plot(check$mPropCon,type="l",ylim=c(0,0.5),lwd=2)
# 
# plot(check$mPCF,type="l",ylim=c(0,1.2),lwd=1)
# abline(h=1)
#  
#propcon2 <- as.numeric(read.table("InOut/PropCon1.csv",header=T,sep=";"))
#points(1:50,propcon2,col="red",lty=2,lwd=2)

# map1 <- read.table("InOut/SmoothMap.txt",header=T,sep="\t")
# rowSums(map1[,3:8])

#sum(apply(map1[3:8],1,max) < 1)/nrow(map1)
